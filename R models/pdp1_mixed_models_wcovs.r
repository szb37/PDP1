...
for (this_measure in unique(df_data$measure)) {
  ...
  model <- lmer('score~(1|pID)+tp+gender+edu+age+LED+severity', subset(df_data, measure==this_measure))
...
write.csv(df_stats, file=paste(export_dir,'/pdp1_mixed_models_wcovs_v1.csv', sep=''), row.names=FALSE)